...
async def scrape_product(
    ...
    for pass_num, allow_tier2 in [(1, False), (2, True)]:
        if pass_num == 2 and _corroboration_satisfied(scraped_data):
            break
        if pass_num == 2:
            pending_domains = len([d for d in by_domain if d not in resolved_domains])
            ...
            for source in candidates:
                ...
                if not search_result_mentions_product(result.content, model_number):
                    ...
                    if search_result_mentions_product(result.content, brand_token):
                        ...
                        break  
                    pattern_no_match_count += 1
                    if pattern_no_match_count >= 2:
                        logger.info(
                            f"Domain {domain} search yielded no match for '{model_number}' "
                            f"after 2 candidate patterns; skipping remaining pattern(s)."
                        )
                        domain_resolved = True
                        break
                    logger.debug(
